...
impl PeerManagerActor {
        ...
        if !new_edges.is_empty() {
            ...
            for edge in new_edges.iter() {
                ...
                if let Some(other) = edge.other(&self.my_peer_id) {
                    ...
                    if self.connected_peers.contains_key(&other) {
    ...
    fn highest_height_peers(&self) -> Vec<FullPeerInfo> {
        ...
        for peer in result.iter() {
            if self
                ...
                .is_empty()
            {
                // We suspect that there is an connected peer to which we don't have a route to
    ...
    fn try_stop_active_connection(&self) {
        debug!(target: "network", "Trying to stop an active connection. Number of active connections: {}", self.connected_peers.len());
...
impl Actor for PeerManagerActor {
    ...
    fn started(&mut self, ctx: &mut Self::Context) {
        ...
        // Start connected peer stats querying.
...
impl PeerManagerActor {
    ...
    fn handle_msg_consolidate(
        ...
        if self.connected_peers.contains_key(&msg.peer_info.id) {
            debug!(target: "network", "Dropping handshake (connected Peer). {:?} {:?}", self.my_peer_id, msg.peer_info.id);
            return ConsolidateResponse::Reject;
        }
